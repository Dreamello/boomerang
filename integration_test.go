...
func runChain(t *testing.T, hops, count int, dropRate float64) (*Stats, string, []string) {
	...
	out := &syncBuf{}
	src, err := NewSource(chain, key, 500*time.Millisecond, out, false, "", 0)
	if err != nil {
...
func TestIntegrationSurvivesPartialLoss(t *testing.T) {
	...
	out := &syncBuf{}
	src, err := NewSource([]string{lossy, dest}, key, 300*time.Millisecond, out, false, "", 0)
	if err != nil {
		t.Fatal(err)
	}
	defer src.Close()
...
func TestIntegrationStopSignal(t *testing.T) {
	...
	out := &syncBuf{}
	src, err := NewSource([]string{relay, dest}, key, time.Second, out, false, "", 0)
	if err != nil {
		t.Fatal(err)
	}
	defer src.Close()
...
func TestIntegrationSourceIgnoresForgedReply(t *testing.T) {
	...
	out := &syncBuf{}
	src, err := NewSource([]string{dest}, other, 200*time.Millisecond, out, false, "", 0)
	if err != nil {
		t.Fatal(err)
	}
	defer src.Close()
